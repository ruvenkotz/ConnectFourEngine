...
import copy
class Engine2:
    # This is my second iteration of an engine, it uses a heuristic function regarding one's in a row, two's in a row,
    # and three's in a row in order to find the best move
    ...
    def choose_a_move(self, player, b):
        ...
        if random_testing and moves_played < num_of_random_moves:
            ...
            current_moves = BoardFunctions.legal_moves(b)
            for i in current_moves:
                print(i)
                if i[1] >1 and i[1] < 5:
                    best_move.append(i)
            best_move = (random.choice(best_move), 10000)
